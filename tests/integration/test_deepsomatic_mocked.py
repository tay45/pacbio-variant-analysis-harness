...
from variant_analysis_harness.somatic.deepsomatic.execution import can_resume_attempt, run_deepsomatic_command, supersede_attempt, write_execution_result
from variant_analysis_harness.somatic.deepsomatic.planning import generate_deepsomatic_plan, write_deepsomatic_plan
...
def build_plan(tmp_path, tiny_reference, row_data):
    ...
    return generate_deepsomatic_plan(cfg, somatic_cfg, project_attempt_dir=tmp_path / "somatic", selected=selected, reference=tiny_reference["fasta"], pair_attempt_id="attempt_001", max_concurrent=2)
...
def test_mocked_execution_success_failure_timeout_and_resume(tmp_path, tiny_reference):
    ...
    result = run_deepsomatic_command(argv, cwd=tmp_path, runner=fake_runner())
    assert result.status == "caller_success"
    failed = run_deepsomatic_command(argv, cwd=tmp_path, runner=fake_runner(2, stderr="boom"))
    assert failed.failure_category == "deepsomatic_execution_failed"
    def timeout_runner(argv, **kwargs):
        raise subprocess.TimeoutExpired(argv, timeout=1, output="partial", stderr="late")
    timed = run_deepsomatic_command(argv, cwd=tmp_path, timeout_seconds=1, runner=timeout_runner)
    assert timed.failure_category == "deepsomatic_timeout"
    ...
    status_path.write_text('{"status":"complete","command_signature":"' + result.command_signature + '","output_validation_status":"PASS"}\n', encoding="utf-8")
...
def test_cli_plan_slurm_rerun_report(tmp_path, tiny_reference):
    ...
    assert main(["somatic-snv-plan", "--config", str(config), "--manifest", str(manifest), "--somatic-project-id", "SOMATIC_1"]) == 0
    somatic_dir = tmp_path / "results" / "test_project" / "somatic" / "SOMATIC_1" / "somatic_attempt_001"
    assert (somatic_dir / "deepsomatic_plan.json").exists()
    assert main(["somatic-snv-status", "--somatic-dir", str(somatic_dir)]) == 0
    assert main(["somatic-snv-report", "--somatic-dir", str(somatic_dir)]) == 0
    assert main(["somatic-snv-rerun-manifest", "--somatic-dir", str(somatic_dir), "--output", str(tmp_path / "rerun.tsv")]) == 0
    assert main(["somatic-snv-slurm", "--config", str(config), "--manifest", str(manifest), "--somatic-project-id", "SOMATIC_1"]) == 0


def test_slurm_array_script_and_rerun_manifest(tmp_path, tiny_reference):
    plan = build_plan(tmp_path, tiny_reference, row())
    write_deepsomatic_plan(plan, tmp_path)
    script = write_deepsomatic_slurm_array(plan, tmp_path / "array.sh", max_concurrent=4)
    assert "#SBATCH --array=1-1%4" in script.read_text()
    rows = generate_deepsomatic_rerun_manifest(tmp_path / "deepsomatic_plan.json", tmp_path / "blocked.tsv", status="BLOCKED")
    assert rows == []
