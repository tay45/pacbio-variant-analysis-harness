"""Severus local execution and attempt helpers."""
...
def run_severus_command(
    argv: list[str],
    *,
    cwd: Path,
    timeout_seconds: int | None = None,
    runner: Callable[..., subprocess.CompletedProcess[str]] = subprocess.run,
) -> SeverusExecutionResult:
    start = time.monotonic()
    try:
        completed = runner(argv, cwd=cwd, text=True, capture_output=True, timeout=timeout_seconds, check=False)
    except subprocess.TimeoutExpired as exc:
        return SeverusExecutionResult("caller_failed", 124, str(exc.stdout or ""), str(exc.stderr or ""), time.monotonic() - start, object_signature(argv), "severus_timeout")
    runtime = time.monotonic() - start
    status = "caller_success" if completed.returncode == 0 else "caller_failed"
    failure = "" if completed.returncode == 0 else "severus_execution_failed"
    return SeverusExecutionResult(status, int(completed.returncode), completed.stdout or "", completed.stderr or "", runtime, object_signature(argv), failure)


def write_execution_result(result: SeverusExecutionResult, path: Path) -> None:
...
def can_resume_attempt(status_path: Path, *, expected_command_signature: str) -> bool:
    ...
    return (
        ...
        and status.get("bnd_validation_status") in {"PASS", "WARN", None}
    )
