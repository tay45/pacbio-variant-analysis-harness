"""Severus rerun manifest selection."""
...
def generate_severus_rerun_manifest(
    plan_path: Path,
    output: Path,
    *,
    ...
    for item in plan.get("pair_statuses", []):
        ...
        if analysis_mode and item.get("analysis_mode") != analysis_mode:
            continue
        rows.append({key: item.get(key, "") for key in ("pair_id", "subject_id", "analysis_mode", "tumor_sample_id", "normal_sample_id", "failure_category")})
    ...
    (output.with_suffix(output.suffix + ".recommendations.json")).write_text(
        json.dumps(
            {
                "selected": len(rows),
                "recommendation": "review Severus output, VCF, BND, QC, and failure category before rerun; no automatic submission performed",
            },
            indent=2,
        )
        + "\n",
